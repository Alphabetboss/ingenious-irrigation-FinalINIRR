...
def iou(box: np.ndarray, boxes: np.ndarray) -> np.ndarray:
    inter = (np.minimum(boxes[:,2], box[2]) - np.maximum(boxes[:,0], box[0])).clip(0) * \
            (np.minimum(boxes[:,3], box[3]) - np.maximum(boxes[:,1], box[1])).clip(0)
...
class YoloV8ONNX:
    ...
    def infer(self, img_bgr: np.ndarray,
        ...
        img = (img.astype(np.float32) / 255.0).transpose(2,0,1)[None]  # 1x3xHxW

        # Inference
        out = self.sess.run(None, {self.inp_name: img})[0]  # shape (1, no, N) or (1, N, no)
...
def hydration_score(dets: List[Dict], size: Tuple[int,int]) -> float:
    """Return 0–10 (0=dry, 5=ok, 10=oversaturated). Uses area fractions per class."""
    w, h = size
    area = w * h + 1e-6
    ...
    for d in dets:
        ...
        name = d["class_name"]
        c = d["confidence"]; confs.append(c)
        if name in water_like:   frac["water"]   += a
        if name in dry_like:     frac["dry"]     += a
        if name in healthy_like: frac["healthy"] += a

    # Simple weighted blend → clamp to [0,10]
...
if __name__ == "__main__":
    ...
    yolo = YoloV8ONNX()
    res = yolo.infer(img)
    print(json.dumps({"hydration_score": res["hydration_score"], "n": len(res["detections"])}, indent=2))
